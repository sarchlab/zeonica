...
type stageConfig struct {
	...
	program   string
	gemmStart int
	gemmEnd   int // exclusive
	reluStart int
	reluEnd   int // exclusive; reluStart==reluEnd means none
}
...
func runStagedMLP(archSpecPath string) (int, stagedRunSummary) {
	...
	stages := []stageConfig{
		{name: "stage1", program: "stage1_gemm_relu.yaml", gemmStart: 0, gemmEnd: 4, reluStart: 4, reluEnd: 6},
		{name: "stage2", program: "stage2_gemm_relu.yaml", gemmStart: 6, gemmEnd: 10, reluStart: 10, reluEnd: 12},
		{name: "stage3", program: "stage3_gemm.yaml", gemmStart: 12, gemmEnd: 16, reluStart: 0, reluEnd: 0},
	}
	...
	for idx, st := range stages {
		...
		northData := make([]uint32, 0, rounds*(st.gemmEnd-st.gemmStart))
		for t := 0; t < rounds; t++ {
			for col := st.gemmStart; col < st.gemmEnd; col++ {
				northData = append(northData, uint32(northPerCol[col][t]))
			}
		}
		...
		driver.FeedIn(northData, cgra.North, [2]int{st.gemmStart, st.gemmEnd}, st.gemmEnd-st.gemmStart, "R")
		...
		summary.Stages = append(summary.Stages, stageRunStats{
			...
			Program:          st.program,
			StartCycle:       0,
			EndCycle:         reportResult.TotalCycles,
	...
	if totalMismatch == 0 {
		fmt.Println("✅ Staged MLP output stream matches CPU reference")
	} else {
		fmt.Printf("❌ Staged MLP mismatches: %d\n", totalMismatch)
	}
	...
	for row := 0; row < height; row++ {
		...
		for t := 0; t < rounds; t++ {
			...
			for col := 0; col < fabricColumns; col++ {
				switch {
				case col >= st.gemmStart && col < st.gemmEnd:
					v = v * northPerCol[col][t]
				case col >= st.reluStart && col < st.reluEnd:
					if v < 0 {
						v = 0
					}
				default:
...
func main() {
	...
	mismatch, summary := runStagedMLP(archSpecPath)
	passed := mismatch == 0
	...
	fmt.Printf("unified staged stats saved: %s\n", unifiedPath)
